...
//     const shipping = appliedCoupon?.discountType === "shipping" ? 0 : 50;
...
//       discount,        // ← new
//       appliedCoupon,
//        walletBalance: wallet.balance,   // ← new
//       total
//     });
...
//     const tax           = parseFloat((subtotal * 0.05).toFixed(2));
//     const total         = parseFloat((subtotal + shipping + tax).toFixed(2));
//     const amountInPaise = Math.round(total * 100); // ✅ Razorpay needs integer paise
...
//   try {
//     const userId    = new mongoose.Types.ObjectId(req.session.user || req.session.userId);
//     const { addressId } = req.body;
...
//         v => v._id.toString() === cartItem.variantId?.toString()
//       );
//       if (!variant) return res.json({ success: false, message: `Variant not found for "${product.name}"` });
...
//       userId,
//       total,
//       `Payment for order`,  // orderId added after order creation below
//       null
//     );
...
//                 'transactions.$.description': `Payment for order #${order._id.toString().slice(-6).toUpperCase()}` } },
...
//   }
// }
// // In admincontroller.js (inside the admin object)
...
//     const userId  = req.session.userId;
//      console.log('cancelOrder — orderId:', id);
//     console.log('cancelOrder — userId from session:', userId);
